...
def solution(start, length):
	...
	for r in range(0, length):
		firstIDinRow = start + r*length
		lastIDinRow = firstIDinRow + lineSize
		checksum ^= XOR(firstIDinRow, lastIDinRow)
		lineSize -= 1
	
	return checksum


# Get XOR of integers in [a,b]
def XOR(a, b):

	'''
	Get XOR of integers in [0,n]
	---------------------------
	(this is nested because it's 
	always called when XOR() is called)
	'''
	def XOR_first_n(n):
		output = {
			0 : n,
	...
	return XOR_first_n(b) ^ XOR_first_n(a-1) # equals XOR([a:b])



print(solution(17,40000))
